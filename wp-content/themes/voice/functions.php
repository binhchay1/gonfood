...
function vce_load_styles() {
	...
	if ( vce_get_option( 'rtl_mode' ) ) {
		...
		if ( $vce_rtl ) {
			wp_register_style( 'vce_rtl', CSS_URI . '/rtl.css', array( 'vce_style' ), THEME_VERSION, 'screen, print' );
...
function vce_load_scripts() {
	...
	if ( !vce_get_option( 'min_js' ) ) {
		...
		wp_enqueue_script( 'vce_picturefill', JS_URI . '/picturefill.js', array( 'jquery' ), THEME_VERSION, true );
...
function vce_load_admin_scripts() {
	...
	if ( $pagenow == 'post.php' || $pagenow == 'post-new.php' ) {
		if ( $typenow == 'post' ) {
			...
			wp_enqueue_script( 'vce_post_metaboxes', JS_URI.'/metaboxes-page.js', array( 'jquery', 'jquery-ui-dialog', 'jquery-ui-sortable', 'jquery-ui-autocomplete' ), THEME_VERSION );
			wp_localize_script( 'vce_post_metaboxes', 'vce_js_settings', vce_get_admin_js_settings() );
