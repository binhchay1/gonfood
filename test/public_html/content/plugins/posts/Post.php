...
class Post
{
	...
	public function insert() {
		...
		$conn = open_connection();
		$sql = "INSERT INTO posts ( created_date, title, content, slug, thumbnail_url, extra_fields ) VALUES ( FROM_UNIXTIME(:created_date), :title, :content, :slug, :thumbnail_url, :extra_fields )";
		$st = $conn->prepare ( $sql );
	...
	public function update() {
		...
		$conn = open_connection();
		$sql = "UPDATE posts SET title=:title, content=:content, slug=:slug, thumbnail_url=:thumbnail_url, fields=:fields, extra_fields=:extra_fields WHERE id = :id";
		$st = $conn->prepare ( $sql );
		$st->bindValue( ":title", $this->title, PDO::PARAM_STR );
		$st->bindValue( ":content", $this->content, PDO::PARAM_STR );
		$st->bindValue( ":slug", $this->slug, PDO::PARAM_STR );
