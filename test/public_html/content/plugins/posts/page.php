...
<div class="section">
	<?php if($action == 'new-post'){ ?>
		...
		<form id="plugin-form-newpost" method="post">
			...
			<div class="row">
				<div class="col-md-8">
					<div class="mb-3">
						<label class="form-label" for="title"><?php _e('Post Title') ?>:</label>
					...
					<div class="mb-3">
						<label class="form-label" for="slug"><?php _e('Post Slug') ?>:</label>
					...
					<div class="mb-3">
						<div class="row">
							<div class="col-md-6">
								<label class="form-label" for="thumb"><?php _e('Post Thumbail') ?>:</label>
							...
							<div class="col-md-6">
								<label class="form-label"><?php _e('Or upload Image') ?></label>
								<input type="file" class="form-control" id="image-file" accept=".png, .jpg, .jpeg, .gif">
							</div>
						</div>
					</div>
					<div class="mb-3">
				...
				<div class="col-md-4">
					<?php
					$extra_fields = get_extra_fields('post');
						...
						<div class="extra-fields">
							...
							foreach ($extra_fields as $field) {
								...
								<div class="mb-3">
									...
									<?php
									$default_value = $post_data->getExtraField($field['field_key']);
									$placeholder = $field['placeholder'];
									if($field['type'] === 'textarea'){
										echo '<textarea class="form-control" name="extra_fields['.$field['field_key'].']" rows="3">'.$default_value.'</textarea>';
									} else if($field['type'] === 'number'){
	...
	<?php } else if($action == 'list'){ ?>
		<a href="dashboard.php?viewpage=plugin&name=posts&action=new-post">
			<button class="btn btn-primary btn-md"><i class="fa fa-plus mr-2" aria-hidden="true"></i> <?php _e('New Post') ?></button>
		</a>
		<a href="<?php echo get_permalink('post'); ?>" target="_blank">
			<button class="btn btn-secondary btn-md"><?php _e('Visit Post Page') ?></button>
		</a>
		<div class="mb-3"></div>
	...
	$(document).ready(()=>{
		...
		$( "form" ).submit(function( event ) {
			...
			arr.forEach(function(item) {
			    if(item.name.startsWith("extra_fields[")) {
			        // Extract the key name by removing "extra_fields[" and the trailing "]"
			        var key = item.name.replace("extra_fields[", "").replace("]", "");
			        extra_fields[key] = item.value;
			    }
			});
		...
		$( "#newposttitle" ).on("input", function() {
			$( "#newpostslug" ).val(($(this).val().toLowerCase()).replace(/\s+/g, "-"));
		});
