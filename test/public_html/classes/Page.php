...
class Page
{
	...
	public function insert() {
		...
		$conn = open_connection();
		$sql = "INSERT INTO pages ( createdDate, title, content, slug, extra_fields ) VALUES ( FROM_UNIXTIME(:createdDate), :title, :content, :slug, :extra_fields )";
		$st = $conn->prepare ( $sql );
	...
	public function update() {
		...
		$conn = open_connection();
		$sql = "UPDATE pages SET title=:title, content=:content, slug=:slug, fields=:fields, extra_fields=:extra_fields WHERE id = :id";
		$st = $conn->prepare ( $sql );
		$st->bindValue( ":title", $this->title, PDO::PARAM_STR );
		$st->bindValue( ":content", $this->content, PDO::PARAM_STR );
